...
class HanoiApp:
    ...
    def __partie_gagnee(self):
        return len(self.__piles[len(self.__piles) - 1].elements) == self.__nombre_disques
    ...
    def start(self):
        ...
        cpt = 1
        while not self.__partie_gagnee():
            self.afficher_piles()
            self.__demander_mouvement()

            cpt += 1

        print('Partie gagnée en {0} coups'.format(cpt))
